...
// Code for reading clock time
{
  ...
  RTC.readTime();
    if (NewRTCh != RTC.h) 
  {                                                                     // H O U R S
  sprintf(psz, "%02d", RTC.h);    // write hours to psz                      
  matrix.setCursor(2, 4); 
  matrix.setTextSize(2);
    matrix.setTextColor(matrix.Color333(3,0,1));          // 
  matrix.fillRect(2, 4, 64, 14, matrix.Color333(0, 0, 0));  // start at (x=2,y=4down) 

    matrix.print(szTime);
  sprintf(psz, "%02d", RTC.m);    // write minutes to psz
  matrix.setCursor(28, 4); 
    matrix.setTextColor(matrix.Color333(3,0,1));
  matrix.fillRect(25, 4, 25, 14, matrix.Color333(0, 0, 0)); // fill with black
    matrix.print(szTime);
  ...
  matrix.fillRect(52, 1, 11, 7, matrix.Color333(0, 0, 0));
    ...
    else if (NewRTCm != RTC.m)                                 // M I N U T E S
  {
  ...
  matrix.fillRect(52, 9, 11, 7, matrix.Color333(0, 0, 0));                  
    ...
    else if (NewRTCs != RTC.s/10)                             // S E C O N D S              
  {
  matrix.setCursor(21, 4); 
  matrix.setTextSize(2);
  matrix.setTextColor(matrix.Color333(7, 0, 7));
